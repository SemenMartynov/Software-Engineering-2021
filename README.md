...
- Lunyak.NA
    ...
    - HW2:
        ...
        * Форс-пуши в опубликованные ветки - это, конечно, плохо, но я все равно их делал, потому что хотел видеть чистую историю(
        * Запрет на мердж без предварительного успешного пайплайна я не сделал, потому что, как я понял, в BitBucket можно попросить `N=const` мердж-чеков, а если надо запретить мердж, пока все чеки не пройдут, то тут [нужен премиум-план](https://support.atlassian.com/bitbucket-cloud/docs/suggest-or-require-checks-before-a-merge/). При пулл реквесте пайплайн просто запускается.
- Ткаченко Даниил
